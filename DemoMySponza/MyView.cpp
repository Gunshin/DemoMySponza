...
int height)
{
    ...
    {
        // shadows woooo
        glBindTexture(GL_TEXTURE_2D, shadowDepthTO);
        glTexImage2D(
            GL_TEXTURE_2D,
            0,
...
windowViewRender(std::shared_ptr<tygra::Window> window)
{
...

    {
        ...
        for (unsigned int i = 0; i < spotLights.size(); ++i)
        {
            ...
            glm::mat4 finalLightProjectionMatrix = biasMat;
            ...
            if (spotLights[i].getCastShadow())
            {
                ...
                glEnable(GL_DEPTH_TEST);// enable the depth test for use with lights
                glDepthMask(GL_TRUE);// disable depth writes since we dont want the lights to mess with the depth buffer
                glDepthFunc(GL_LEQUAL);

                glClear(GL_DEPTH_BUFFER_BIT); // clear all 3 buffers
            ...
            glUniformMatrix4fv(glGetUniformLocation(spotLightProgram.getProgramID(), "shadowBiasProjectionViewMat"), 1, GL_FALSE, glm::value_ptr(finalLightProjectionMatrix));
    ...
    bool drawDoF = ((MyController*)(tygra::Window::mainWindow()->controller().get()))->getDrawDoF(); // hacky because i dont know how i was supposed to do it =_=
...

    {
        ...
        GLuint textureToDraw = drawDoF ? depthVerticalPassTO : lbufferTO;
